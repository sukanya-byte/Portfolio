...
// ── Thread rail ──
var THREAD_STOPS=['top','ground','challenge','approach','workflow','decisions','impact','more'];
...
// stage entrance observer
function initStageReveal(){
  var stages=document.querySelectorAll('.stage');
  if(NO_MOTION || !('IntersectionObserver' in window)){
    stages.forEach(function(st){st.classList.add('seen');});return;
  }
  var io=new IntersectionObserver(function(entries){
    entries.forEach(function(e){
  ...
  setTimeout(function(){stages.forEach(function(st){st.classList.add('seen');});},3000);
}
